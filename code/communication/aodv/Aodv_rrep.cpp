...
Aodv_rrep::Aodv_rrep(int hop_count, std::string source_ip, std::string dest_ip, std::string last_hop, int dest_seq, int life_time, int ttl): Aodv_message(dest_ip, dest_seq, ttl){
...
std::string Aodv_rrep::to_string(){
	...
	result = "RREP;" + result;
	result += ";" + std::to_string(m_hop_count) + ";" + m_source_ip + ";" + std::to_string(m_life_time) + ";" + m_last_hop;
	return result;
}
