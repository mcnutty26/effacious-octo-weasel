...
Ad Hoc On-Demand Distance Vector (AODV) is a non-position based protocol which enables dynamic, self-starting, multi-hop routing between participating mobile nodes. It operates by sending Route Requests (RREQs), Route Replies (RREPs) and Route Errors (RERRs) messages between nodes, which are used to establish endpoints of a communication connection.  In AODV, the source node and the intermediate nodes store the next top information corresponding to each flow for data packet transmission \cite{perkins2003}.
AODV is a reactive (on-demand) routing protocol in that it does not do anything if a route already exists between communicating nodes, so connection setup delay is less, and destination sequence numbers are used to find the latest route to the destination . The AODV protocol also keeps the overhead of messages small, and has a great advantage in overhead over simple protocols which need to keep the entire route from source to destination in their messages. This protocol is also loop-free and avoids the counting to infinity problem through the use of sequence numbers. Despite this, such protocols have a high latency time in finding a route, and excessive flooding can lead to network clogging. 

\subsection{Dynamic Source Routing}
\subsection{Greedy Perimeter Stateless Routing}
\subsection{Distributed Efficient Multi-Hop Clustering Protocol}	
